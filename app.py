# app.py — Amount + Date Finder (by BANK sheets; SIB/SAIB; Hijri-aware; fixed boolean logic)
...
def _detect_bank(filename: str, sheetname: str, df: pd.DataFrame) -> str:
    """Detect bank from file name, sheet name, headers, or top rows."""
...
def _normalize_ledger(df: pd.DataFrame, amount_cands: List[str], date_cands: List[str],
    ...
    if enable_debit_credit:
        if credit_sar and debit_sar:
            ...
            if debit_col is None:
                for c in df.columns:
                    lc = str(c).lower()
                    if "debit" in lc or "مدين" in lc:
                        debit_col = c; break
            if credit_col is None:
                for c in df.columns:
                    lc = str(c).lower()
                    if "credit" in lc or "دائن" in lc:
                        credit_col = c; break
    ...
    if credit_col and debit_col:
        ...
        if amt_col is None:
            for c in df.columns:
                lc = str(c).lower()
                if any(tok in lc for tok in ["amount","credit amount","debit amount","balance","balance (sar)","المبلغ","الرصيد"]):
                    amt_col = c; break
        if amt_col is None:
            guess = _guess_amount_column(df)
            if guess:
                amt_col, info["amt_auto"] = guess, True
        if amt_col:
...
if run_btn:
    ...
    for (fname, fbytes) in files:
        ...
        for sname, df in sheets.items():
            ...
            diag.append({
                "File": fname, "Sheet": sname, "Bank": bank_code,
    ...
    for idx, r in df_in.iterrows():
        ...
        for fname in repo:
            for sname, (d, bank_code) in repo[fname].items():
                ...
                # --- Build boolean masks safely (fixes TypeError from operator precedence) ---
                m1 = (series - amt).abs() <= tol                # +amount on net/signed
                m2 = (series + amt).abs() <= tol                # -amount on net/signed
