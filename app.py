# app.py — Local Statements: Header Model + Search/Pair + Raw Rows + Excel Export
# -------------------------------------------------------------------------------
...
# Final normalized schema (for template/export + UI)
...
def normalize_df_with_raw_and_mapping(raw: pd.DataFrame, filename_hint: str) -> Tuple[pd.DataFrame, Dict[str, str]]:
    """
    Normalize to common columns, attach full raw row (_rawdata), and return
    a mapping dict {normalized_field: source_column_name or ''}.
    """
    # Build mapping dict of which source col matched which normalized field
    mapping: Dict[str, str] = {}

    # Which source header matched
    def matched(candidates_key: str) -> str:
        col = pick_col(raw, HEADER_MAP[candidates_key])
        mapping[candidates_key] = col or ""
        return col

    # Save raw JSON per row to preserve original data
    ...
    if c_amt is not None and c_deb is None and c_cred is None and c_drcr is not None:
        ...
        df["amount"] = amount  # already signed or unsigned (assume signed)
    else:
    ...
    # ensure model column order + add source
    for col in MODEL_COLUMNS:
        if col not in df.columns:
            df[col] = pd.NA
    # 'source' will be added by caller
    return df[MODEL_COLUMNS[:-2] + ["_rawdata"]], mapping  # (exclude 'source' here)
...
if "_headers" not in st.session_state:
    st.session_state._headers: Dict[str, List[str]] = {}
if "_mappings" not in st.session_state:
    st.session_state._mappings: Dict[str, Dict[str,str]] = {}
...
with colh1:
    if st.button("Show per-file original headers"):
        if not st.session_state._headers:
            ...
            # Build a small table: File | Original Headers (comma-separated)
...
with colh2:
    if st.button("Show detected mappings (→ normalized model)"):
        if not st.session_state._mappings:
            ...
            # Flatten mapping dicts
            records = []
            for fn, mp in st.session_state._mappings.items():
                # map keys in HEADER_MAP to model-friendly names
                rename = {
                    "date":"date", "narr":"narration", "debit":"debit", "credit":"credit",
...
# Export model & mappings to Excel
if st.button("📤 Export Header Model (Excel)"):
    if df_all.empty or not st.session_state._headers:
        ...
        model_template = pd.DataFrame({
            "Normalized Field": ["date","bank","account","narration","ref","amount","abs_amount","balance","direction","source","_rawdata"],
            "Type / Notes": [
                "date (YYYY-MM-DD)",
                "bank short code (SNB, SABB, etc.)",
                "account/IBAN",
                "transaction text",
                "reference/trace/UTR",
                "signed number (+credit, -debit)",
                "absolute amount (for pairing)","running balance",
                "FROM (OUT) or TO (IN)","source file name",
                "JSON of the original row"
            ]
        })
...
# ---------- Search / Pair ----------
st.subheader("🔎 Search by Amount & Pair (optional)")
    ...
    if df_all.empty:
        ...
        if target is None:
            ...
            if not pairs.empty:
                ...
                # Excel download (FROM, TO, and Pairs with raw rows)
                outbuf = io.BytesIO()
                with pd.ExcelWriter(outbuf, engine="openpyxl") as xw:
